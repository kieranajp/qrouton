...
- `internal/vault/`: the Obsidian vault index. It walks one root for Markdown, chunks it at headings into ~500-character windows, ranks with BM25 and Ollama embeddings fused by RRF, and caches vectors per profile. The desktop process holds it in memory and answers the vault tools over the control socket; `workbench` carries its types on the wire, so it imports nothing of qrouton's but `atomicfile`.
