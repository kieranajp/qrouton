...
- `internal/config/`: config file and XDG paths. `Load` never prompts and never fails for a missing value — a zero-repo session needs neither a root nor owners, so the root defaults and an empty owner list is simply an empty repository list. A workbench opening on no session asks for the owners and the sessions root once, then marks the config `welcomed`; one opening on a session never asks, so an install that always resumes stays unasked. `thoughts` names the default thoughts root, derived from the sessions root when unset, and the extra roots a session is routed to when every one of its repos' orgs maps there.
- `internal/vault/`: the thoughts index, one per thoughts root. Each walks its root for Markdown, chunks it at headings into ~700-character windows, ranks with BM25 and Ollama embeddings fused by RRF, and caches vectors per root. The desktop process holds them in memory and answers the thoughts tools over the control socket from the one root the session's `thoughts` link resolves into. `config` validates its roots through `vault`, and `workbench` carries its types on the wire, so it imports only the shared leaves.
...
- Focus is never taken *from* the user. An agent surface is a tab and leaves the keyboard where it was, and the conversation is the only desktop window an agent opens. A surface the user asked for may be a desktop window of its own and may take the keyboard; one the user asked for to watch rather than drive, such as speaker notes on a second display, opens without it.
