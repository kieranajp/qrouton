...
- `cmd/mcp/`, `cmd/agents/`, `cmd/mode/`, `cmd/agent/`, `cmd/shell/`: `*cli.Command` definitions (flags only) delegating to `internal/*`.
...
- `internal/mcpserver/`, `internal/agents/`: the agent's window tools; and the Claude subagent hook collector, which is a write path only — the log has no reader yet, so nothing scans it.
