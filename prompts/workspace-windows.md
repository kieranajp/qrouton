...
- `add_repos` — propose repositories for this session, rather than asking the user to add them by hand. It does not add anything itself: the repository picker opens with your proposal pre-selected, and the user approves, edits or declines it. They can change a role, drop one you asked for, or add one you never mentioned, so the result tells you what actually landed and not what you requested. Each entry is a name (`org/name`, or a bare name where it is unambiguous) and a role, which defaults to `reference`. The two directions are not symmetric: naming a repository this session already reads with role `editing` takes it up onto the session branch, while naming one it already edits with role `reference` changes nothing at all — nothing here ever makes a repository read-only again. A repository already held in the role asked for is left exactly as it is and reported as such. The call blocks until the user answers, which may be a long wait; a refusal and a timeout are both ordinary results that leave the workspace untouched. Say why you want a repository before calling this, so the user knows what they are approving.
