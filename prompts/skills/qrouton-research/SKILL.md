...
2. When `search_thoughts` is available, search the thoughts root for each approved question before delegating, always with `kinds: ["research"]`. Only research artifacts may reach the lead: specs, plans and notes carry intended solutions, whatever their workstream or ticket. The scores are rank evidence, not relevance, so read a hit with `read_thoughts` and keep it only when it bears on the question. Pass a reference, not content: the artifact ID, heading breadcrumb, line range, and the approved question it bears on. Add nothing else: no summary, no staleness note, no comment on its claims, and no reason why the hit matters to the ticket. The lead verifies prior research against live code without being told. No hits, an unavailable channel and a list of irrelevant neighbours all mean the same thing: pass nothing and let the lead investigate fresh.
3. Spawn a `qrouton-research-lead` when available, otherwise a general subagent. Give it only that document's path/content, the thoughts references you kept with the plain instruction to read them with `read_thoughts` (the lead has it; do not hedge on its availability), safe context pointers, the active/reference repo rules, and the instruction to fill it in where it stands.
