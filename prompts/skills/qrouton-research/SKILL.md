...
2. Spawn a `qrouton-research-lead` when available, otherwise a general subagent. Give it only that document's path/content, safe context pointers, the active/reference repo rules, and the instruction to fill it in where it stands.
...
4. Research describes what is; it does not recommend a solution.
