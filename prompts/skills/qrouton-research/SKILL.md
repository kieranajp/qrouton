...
3. Spawn a `qrouton-research-lead` when available, otherwise a general subagent. Give it only that document's path/content, the vault references you kept with the plain instruction to read them with `read_vault` (the lead has it; do not hedge on its availability), safe context pointers, the active/reference repo rules, and the instruction to fill it in where it stands.
