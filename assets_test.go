...
func TestStampAssetsWritesOverwritesAndRespectsOwnership(t *testing.T) {
	...
	if cb, err := os.ReadFile(filepath.Join(dir, ".codex", "agents", "qrspi-researcher.toml")); err != nil {
		t.Fatal("Codex researcher adapter not stamped:", err)
	} else if !strings.HasPrefix(string(cb), "# Generated by qrouton") {
		t.Fatal("Codex researcher adapter does not have valid TOML comment marker")
	}
