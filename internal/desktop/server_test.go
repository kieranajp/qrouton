...
func TestDispatchRefusesEachOperationWithItsOwnSentinel(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{name: "add repos on a session-less socket",
			req: workbench.Request{Op: workbench.OpAddRepos,
				AddRepos: &workbench.AddReposRequest{Repos: []workbench.RepoAddition{{Name: "org/svc"}}}},
			want: ErrNoSession},
		{name: "add repos with no deadline", owner: owner,
			req: workbench.Request{Op: workbench.OpAddRepos,
				AddRepos: &workbench.AddReposRequest{Repos: []workbench.RepoAddition{{Name: "org/svc"}}}},
			want: ErrNoAddReposDeadline},
		{name: "add repos without a payload", owner: owner,
			req: workbench.Request{Op: workbench.OpAddRepos}, want: ErrNoAddRepos},
		{name: "add repos naming nothing", owner: owner,
			req: workbench.Request{Op: workbench.OpAddRepos,
				AddRepos: &workbench.AddReposRequest{}}, want: ErrNoAddRepos},
		{name: "add repos on a workbench with no picker", owner: owner,
			req: workbench.Request{Op: workbench.OpAddRepos,
				AddRepos: &workbench.AddReposRequest{
					Repos:    []workbench.RepoAddition{{Name: "org/svc"}},
					Deadline: time.Now().Add(time.Minute),
				}},
			want: ErrNoRepositoryAdd},
