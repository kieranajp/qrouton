...
window.wailsCall = async (name, input, key) => {
 ...
 orgs: ["acme"],
  ...
  if (name.endsWith(".PreviewVaultCorpus")) {
    ...
    return {id:"corpus-preview",targetProfile:input.targetProfile,total:300,ready:297,repairRequired:2,excluded:1,entries:Array.from({length:300},(_,i)=>({key:`corpus${i}`,assetCount:i<2?1:0,assetBytes:i<2?68:0,id:`personal/R${i}`,name:`R${i}.md`,session:i===298?"ambiguous":"personal",kind:"research",title:`Finding ${i}`,destination:i===299?"private":input.targetProfile,disposition:i===299?"excluded":i>=297?"repair":"ready",dependencies:i===0?["corpus1"]:[],unknownRevisions:true,repairs:i>=297&&i<299?[{field:"repos",reason:"Repository scope needs review"}]:[],warnings:[],reason:i===299?"Different destination vault":""}))};
  }
  if (name.endsWith(".VaultCorpusEntry")) {importCalls.push({action:"detail",key});return {key,name:key+".md",assets:[{key:"png1",href:"../../assets/capture.png",path:"personal/assets/hash.png",hash:"hash",size:68,width:2,height:2}],document:{schemaVersion:1,id:"personal/R1",session:"personal",kind:"research",title:"Finding",author:"Local Author",date:"2026-09-22",workstream:"personal",state:"active",repos:[]},body:"Original evidence",canonical:"Canonical evidence",destination:"shared",path:"personal/R1.md",repairs:[],warnings:[],provenance:{author:{source:"batch",detail:"reviewed default"}}};}
