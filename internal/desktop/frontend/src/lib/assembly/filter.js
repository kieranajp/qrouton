 ...
 * cache; only the shown answers to the cap.
 * @param {{repos?: Repo[], owners?: string[], query?: string, cap?: number}} [options]
 * @returns {{rows: Row[], shown: number, total: number}}
 */
export function filter({ repos = [], owners = [], query = "", cap = 0 } = {}) {
