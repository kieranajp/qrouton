...
func TestFailedAdditionCanRetryWithoutReplacingCompletedWorktrees(t *testing.T) {
	...
	if len(got.Repos) != 1 || got.Repos[0].Name != first.Name || got.Picker != nil || got.Mode != session.ModeAssistant {
	...
	if len(got.Repos) != 2 || got.Mode != session.ModeRPI || got.Picker == nil {
