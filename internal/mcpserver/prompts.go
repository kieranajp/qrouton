...
const serverInstructions = "Drive the user's qrouton workbench. These tools open windows beside your conversation, docked as tabs in the session's right pane — or as separate desktop windows the user moves and closes, if that is their preference. Opening one leaves the keyboard where it is, so the user can watch while you work and keep chatting. A tab whose command exits cleanly goes with it; one whose command fails stays so the error remains readable, and either way the tab says whether its process is running, succeeded or failed. Use open_file to show a document (especially after creating one); run_command to run long-lived or noisy work (dev servers, watchers, builds, log tails) where the user can see it instead of in your own shell — that window is interactive, so Ctrl-C there reaches the process; read_window to inspect its output; show_diff to display a repo's changes for review; notify to get the user's attention when you finish or need them; close_window/list_windows to manage what you have open; escalate to hand a piece of work off to the full RPI workflow once you've drafted a brief. escalate is the one tool that takes the keyboard, and the one that always gets a desktop window, because the repository picker needs both. All paths and working directories must belong to this session."
...
// Tool descriptions.
const (
	descOpenFile = "Show the user an existing session file. A markdown file is rendered as a formatted pane — headings, task lists and highlighted code — and anything else opens in their configured terminal editor at the given line. Either way it becomes the selected tab, so it is not left behind another one, and the keyboard stays with the conversation. A rendered document stays open for reference; an editor's tab closes when the user quits it. Use this after writing a document when showing it to the user is helpful."

	descRunCommand = "Run a shell command in a window of its own instead of your own shell. Ideal for long-running or noisy processes (dev servers, test watchers, builds, log tails) the user should see live: it is interactive, so Ctrl-C there reaches the process. The keyboard stays with the conversation, reusing a name replaces that window, and a command that succeeds closes it while one that fails leaves it open with its error. Its tab reports whether the command is running, succeeded or failed. Read its output later with read_window."
	...
	descListWindows = "List the windows qrouton is holding for you, by name, whether they are tabs in the session pane or desktop windows."
