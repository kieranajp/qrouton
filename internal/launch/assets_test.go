...
func TestStampAssetsWritesOverwritesAndRespectsOwnership(t *testing.T) {
	...
	for _, root := range []string{".claude", ".agents"} {
		...
		if !strings.Contains(string(rb), "Generated by qrouton") {
			t.Fatalf("%s missing generated-by marker", reference)
		}
		if !strings.Contains(string(rb), "### Verify") {
			t.Fatalf("%s does not carry the plan template", reference)
		}
	}
	agentNames := []string{
